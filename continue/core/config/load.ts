...
// Stage 3: CustomLLM provider removed. Legacy config.json code paths below
// now throw if the user's config tries to declare a `customLLM`.
const CustomLLMClass: any = class {
  constructor() {
    throw new Error(
      "customLLM providers are not supported in this internal build",
    );
  }
};
