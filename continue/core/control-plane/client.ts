...
export class ControlPlaneClient {
  ...
  private async request(_path: string, _init: RequestInit): Promise<Response> {
    // Stage 2 POC: control-plane is disabled. Fail fast instead of attempting
    // a network call that would hang on DNS resolution of a dead URL. Callers
    // already have try/catch around hub calls, so they fall back cleanly.
    throw new Error("Continue hub disabled in this build");
  }
