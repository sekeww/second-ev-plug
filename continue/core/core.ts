...
// Stage 3: Lemonade provider removed.
import { fetchModels } from "./llm/fetchModels";
// Stage 3: Ollama provider removed — all traffic goes through the gateway (OpenAI provider).
  ...
  private async handleListModels(msg: Message<{ title: string }>) {
    ...
    try {
      if (model) {
        ...
        // Stage 3: provider-specific listModels paths removed along with providers.
        return undefined;
      }
    } catch (e) {
