...
// Stage 2 POC:
// - sentryVitePlugin removed (no build-time source-map upload to Sentry)
// - posthog-js + posthog-js/react aliased to local no-op stubs so every
//   import in the GUI resolves to a stub at bundle time.
export default defineConfig({
  plugins: [react(), tailwindcss()],
  resolve: {
    alias: {
      "posthog-js/react": resolve(__dirname, "stubs/posthog-js-react.tsx"),
      "posthog-js": resolve(__dirname, "stubs/posthog-js.ts"),
    },
  },
  build: {
    sourcemap: true,
